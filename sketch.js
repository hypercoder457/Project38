...
function setup() {
  ...
  trex = createSprite(width / 4, height / 4, 20, 50);
  ...
  gameOver = createSprite(width / 2, height / 6);
  gameOver.addImage(gameOverImg);

  restart = createSprite(width / 2, (height / 6) + 40);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(width / 5, 40, 10);
...
function spawnObstacles() {
  if (frameCount % 60 === 0) {
    var obstacle = createSprite((width / 4) + 150, (height / 2), 10, 40);
